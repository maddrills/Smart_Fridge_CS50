    ...
    if request.method == "POST":
        ...
        if stum == "del":
            ...
            try:
                ...
                db.execute("INSERT INTO data(current,privous,reduce,total,code) VALUES(?,?,?,?,?)",0,0,0,0,code_number)
            except:
                print("conflict on data")

            #initalising default values for master = read and old = 0
            try:
                db.execute("INSERT INTO master_weight(master_weight,code) VALUES(?,?)",0,code_number)
            except:
                print("conflict on master_weightr")
            try:
                db.execute("INSERT INTO old_weight(old_weight,code) VALUES(?,?)",0,code_number)
            except:
                print("conflict on old_weight")
                #initialise the db for scaned_tag
            try:
                db.execute("INSERT INTO scaned_tag(scaned_tag,product_code) VALUES(?,?)",nothing,code_number)
            except:
        ...
        if dore == "remove":
            ...
            try:
                ...
                un_updated = db.execute("SELECT product_total FROM repository WHERE product_total > ?",master_weight + 5)
